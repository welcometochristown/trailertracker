...
const parseGPRMC = function(raw) {
  ...
  if (splitted.length !== 12 && splitted.length !== 14) {
	throw new TypeError(`Invalid GPRMC string - length is ${splitted.length} characters`);

  }
 
  if(splitted[0] !== '$GPRMC') {
  	  throw new TypeError('Invalid GPRMC string, doesnt start with $GPRMC');
  }
